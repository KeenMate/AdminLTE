...
(function ($) {
  $.color = {}; $.color.make = function (r, g, b, a) {
    const o = {}; o.r = r || 0; o.g = g || 0; o.b = b || 0; o.a = a != null ? a : 1; o.add = function (c, d) {
    ...
    let res; const m = $.color.make; if (res = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(str)) {
    ...
    } const name = $.trim(str).toLowerCase(); if (name == 'transparent') {
...
(function ($) {
  ...
  Canvas.prototype.getTextInfo = function (layer, text, font, angle, width) {
    let info; let layerCache; let styleCache; let textStyle
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    plot.pointOffset = function (point) {
      return {
        left: parseInt(xaxes[axisNumber(point, 'x') - 1].p2c(Number(point.x)) + plotOffset.left, 10),
        top: parseInt(yaxes[axisNumber(point, 'y') - 1].p2c(Number(point.y)) + plotOffset.top, 10)
      }
    }
    ...
    function parseOptions(opts) {
      ...
      const fontSizeDefault = fontSize ? Number(fontSize.replace('px', '')) : 13
      const fontDefaults = {
    ...
    function fillInSeriesOptions() {
      ...
      for (i = 0; i < series.length; ++i) {
        ...
        if (s.lines.zero == null) {
          s.lines.zero = Boolean(s.lines.fill)
        }
      ...
      for (i = 0; i < series.length; ++i) {
        ...
        if (!format) {
          ...
          if (s.bars.show || s.lines.show && s.lines.fill) {
            const autoscale = Boolean(s.bars.show && s.bars.zero || s.lines.show && s.lines.zero)
            format.push({
              y: true,
              number: true,
              required: false,
    ...
    function allocateAxisBoxFirstPhase(axis) {
      ...
      if (!isNaN(Number(tickLength))) {
        padding += Number(tickLength)
      }
    ...
    function setupGrid() {
      ...
      if (showGrid) {
        const allocatedAxes = $.grep(axes, (axis) => axis.reserveSpace)
    ...
    function setRange(axis) {
      const opts = axis.options
      let min = Number(opts.min != null ? opts.min : axis.datamin)
      let max = Number(opts.max != null ? opts.max : axis.datamax)
      const delta = max - min
    ...
    function drawAxisLabels() {
      $.each(allAxes(), (_, axis) => {
        ...
        let tick; let x; let y; let halign; let valign
    ...
    function drawSeriesLines(series) {
      ...
      function plotLineArea(datapoints, axisx, axisy) {
        ...
        let ypos = 1; let segmentStart = 0; let
          segmentEnd = 0
    ...
    function drawSeriesPoints(series) {
      function plotPoints(datapoints, radius, fillStyle, offset, shadow, axisx, axisy, symbol) {
        const points = datapoints.points; const
    ...
    function drawBar(x, y, b, barLeft, barRight, fillStyleCallback, axisx, axisy, c, horizontal, lineWidth) {
      let bottom; let drawBottom; let drawLeft; let drawRight
      let drawTop; let left; let right; let tmp
      let top
    ...
    function drawSeriesBars(series) {
      ...
      const fillStyleCallback = series.bars.fill ? function (bottom, top) {
    ...
    function findNearbyItem(mouseX, mouseY, seriesFilter) {
      ...
      let item = null; const foundPoint = false; let i; let j; let ps
